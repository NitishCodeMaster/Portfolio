...
import { ArrowRight, Download, Sparkle, MousePointerClick } from "lucide-react";
...
export const Hero = () => {
  const skills = ["Frontend", "React", "Full Stack"];

  return (
    <section id="home" className="relative h-screen w-full overflow-hidden bg-[#020204] text-white flex flex-col items-center justify-center">
      
      {/* Digital Space Background Core System */}
      <BackgroundEffects />
      <FloatingStars />

      {/* Main Structural Layout Grid Frame (Perfect 100vh Alignment) */}
      <div className="relative z-10 mx-auto w-full max-w-4xl px-6 flex flex-col items-center justify-center text-center h-full pt-16">
        ...
        {/* 3. PREMIUM HORIZONTAL TYPOGRAPHY HEADING */}
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative mb-5 w-full select-none">
          {/* Giant Extended Spatial Background Layer Mask */}
        ...
        {/* 4. CLEAN TWO-LINE DESCRIPTION */}
        <motion.p initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="mb-6 max-w-xl text-zinc-400 text-xs md:text-sm leading-relaxed font-light tracking-wide">
        ...
        {/* 5. METADATA SKILL PILLS (Horizontal Configuration Spacing) */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.38 }} className="flex items-center gap-2 mb-8 flex-wrap justify-center">
          {skills.map((skill) => (
            <span key={skill} className="rounded-full border border-white/[0.05] bg-white/[0.02] px-3.5 py-1 text-[10px] font-semibold tracking-wider text-zinc-400 shadow-[inset_0_1px_0_rgba(255,255,255,0.02)] backdrop-blur-md">
              {skill}
            </span>
          ))}
        </motion.div>

        {/* 6. SYSTEM ACTION BUTTONS CTA */}
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.45 }} className="flex flex-col sm:flex-row items-center justify-center gap-3.5">
