...
function ExperienceSection() {
  ...
  return <section className="py-32 relative z-10 bg-[#0a0a0a]" id="experience">
  ...
  >
    ...
    /* Vertical Line */
          ...
          <div className="flex flex-col gap-12">
            {experiences.map((item, i) => <motion.div
    key={i}
    ...
    /* Content Card */
                ...
                <div className={`w-full md:w-1/2 ${i % 2 === 0 ? "md:pl-16" : "md:pr-16 text-left md:text-right"}`}>
                  <div className="glass-card p-8 hover:bg-[#111] transition-colors group relative overflow-hidden">
                    ...
                    {item.tech.length > 0 && <div className={`flex flex-wrap gap-2 ${i % 2 === 0 ? "" : "md:justify-end"}`}>
                        {item.tech.map((t) => <span key={t} className="text-xs text-white/40 bg-white/5 px-2 py-1 rounded">
                            {t}
                          </span>)}
                      </div>}
                  </div>
