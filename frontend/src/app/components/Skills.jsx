...
const Skills = () => {
  ...
  return <section id="skills" className="relative overflow-hidden bg-[#020203] px-6 py-32 text-white">
      ...
      <div ref={bubbleRef} className="pointer-events-none absolute left-0 top-8 h-72 w-72">
        {Array.from({ length: 18 }).map((_, index) => <span
    key={index}
  ...
  >
    ...
    >
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {categories.map((category, index) => <SkillCard key={category.title} category={category} index={index} />)}
        </div>
      </div>
    </section>;
};
function SkillCard({
  ...
  >
      ...
      <div className="flex flex-wrap gap-2">
        {category.skills.map((skill, i) => <motion.span
    key={skill}
